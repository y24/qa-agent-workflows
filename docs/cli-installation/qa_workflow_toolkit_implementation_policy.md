...
`qatool install` 実行後、対象プロジェクトには以下のような構成を作成する。
...
Usage:
  qatool install    Install QA workflow assets into current folder
  qatool list       Show available workflows
...
`qatool install` は、実際にコピーする前にインストール計画を生成する。
...
- `qatool update` 時の安全な上書き判断
- `qatool uninstall` 時の削除対象判断
...
### Step 6: `qatool install` の対話UIを実装する
